...
class NTKMixedScalingRotaryEmbedding(RotaryEmbedding):
    """
        NTKMixedScalingRotaryEmbedding
    """
    ...
    def _compute_cos_sin_cache(self) -> torch.Tensor:
        inv_freq = self._compute_inv_freq(self.base)
        max_len = self.max_position_embeddings * int(self.scaling_factor)
        a = math.log(int(self.scaling_factor)) / ((self.rotary_dim / 2) ** self.scaling_power)
        inv_freq = inv_freq / torch.exp(a * torch.arange(1, self.rotary_dim / 2 + 1,device="cuda").float() ** self.scaling_power)  
        t = torch.arange(max_len, dtype=torch.float, device="cuda")
        freqs = torch.einsum("i,j -> ij", t, inv_freq)
        cos = freqs.cos()
        sin = freqs.sin()
    ...
    if rope_scaling is None:
        ...
        if scaling_type == "linear":
            ...
            rotary_emb = YaRNScalingRotaryEmbedding(head_size, rotary_dim,
                                                    ...
                                                    scaling_factor,
                                                    **extra_kwargs)
        elif scaling_type == "ntkmixed":
            scaling_power = rope_scaling["power"]
            rotary_emb = NTKMixedScalingRotaryEmbedding(
                head_size, rotary_dim, max_position, base, is_neox_style,
                scaling_factor,scaling_power=scaling_power)
        else:
