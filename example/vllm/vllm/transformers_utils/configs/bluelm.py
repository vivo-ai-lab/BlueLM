...
class BlueLMConfig(PretrainedConfig):
    ...
    def _rope_scaling_validation(self):
        ...
        if not isinstance(self.rope_scaling, dict) or len(self.rope_scaling) != 3:
            raise ValueError(
                "`rope_scaling` must be a dictionary with with two fields, `type` , `factor` , `power`, "
        ...
        if rope_scaling_type is None or rope_scaling_type not in ["linear", "dynamic","ntkmixed"]:
            raise ValueError(
                f"`rope_scaling`'s type field must be one of ['linear', 'dynamic','ntkmixed'], got {rope_scaling_type}"
            )
